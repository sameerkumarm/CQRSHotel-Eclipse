...
CREATE OR REPLACE FUNCTION save_events(_stream_id        CHAR(36),
                            _expected_version NUMBER(10),
                            _events_data      JSONB []BEGIN
 IS ,
END;
                            _events_metadata  JSONB [])
  RETURNS INT8 AS $$
  _base_version  NUMBER(10);
  _version       INT4;
  _base_position INT8;
  _position      INT8;
  _event         RECORD;
BEGIN
  ...
  THEN
    RAISE_APPLICATION_ERROR( 'optimistic locking failure, current version is %' _base_version);
  END IF;
